...
export const blockUser=async(req,res)=>{
    try{
        ...
        if(user) return res.status(200).json({status:true,"message":"User Blocked Successfully",user:user})
...
export const blockProduct=async(req,res)=>{
    try{
        ...
        if(product) return res.status(200).json({status:true,"message":"product Blocked Successfully",product:product})
        throw new Error("Internal Error")
    }
    catch(error){
    return res.status(500).json({status:"error",message:error})
...
export const unBlockProduct=async(req,res)=>{
    try{
        ...
        if(product) return res.status(200).json({status:true,"message":"product Unblocked Successfully",product:product})
        throw new Error("Internal Error")
    }
    catch(error){
    return res.status(500).json({status:"error",message:error})
